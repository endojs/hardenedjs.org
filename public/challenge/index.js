...
const $guess = $("#guess");
const $dateNowEndowed = $("#date-now-endowed");
const $guessBox = $("#guess-box");
const $attackerProgram = $("#attacker-program");
...
const enableDateNow = () => {
  ...
  url.searchParams.set("dateNow", "enabled");
  window.history.replaceState(null, "", url.href);
  $dateNowEndowed.checked = true;
};

const disableDateNow = () => {
  delete endowments.Date;
  url.searchParams.delete("dateNow");
  window.history.replaceState(null, "", url.href);
  $dateNowEndowed.checked = false;
};

const setDateNowEnabled = (endowed) => {
  if (endowed) {
    enableDateNow();
  } else {
    disableDateNow();
...
setDateNowEnabled(url.searchParams.get("dateNow") === "enabled");

$dateNowEndowed.addEventListener("change", () => {
  setDateNowEnabled($dateNowEndowed.checked);
});
...
const setMacguffinText = (text) => ($macguffin.innerText = text);
...
const stopAttacker = () => {
  ...
  $guessBox.classList.remove("attacking");
};

const startAttacker = () => {
  enableAttacker = true;
  $guessBox.classList.add("attacking");
};
...
let reportAllStop = () => {};
let stopped = Promise.resolve(undefined);

const setProgram = (program) => {
  $attackerProgram.value = program;
};

const submitProgram = (program) => {
  ...
  function nextGuess() {
    ...
    requestAnimationFrame(nextGuess);
  }
  nextGuess();
};
...
$("#attacker-submit").addEventListener("click", (_event) => {
  ...
  const code = $attackerProgram.value;
...
// IMPORTANT: These functions are stringified and evaluated in the compartent.
// Although we do not call these functions, embedding their source exposes them
// to the linter and prettifier.
...
$("#sample-timing-exposed").addEventListener("click", () => {
  setProgram(`${timing}`);
  enableDateNow();
});

$("#sample-timing-blocked").addEventListener("click", () => {
  setProgram(`${timing}`);
  disableDateNow();
});
