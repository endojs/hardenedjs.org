...
- `Date.now()`, no-argument calls to the `Date` constructor, `Math.random()`, mutable static properties of the `RegExp` constructor. Starting with Google-Caja in the 2000s, these have been locked down. We're confident these are not non-overt channels in Hardened JS and in the ses-shim and XS implementations of Hardened JS.
...
- Node's [async_hooks](https://nodejs.org/api/async_hooks.html) adding unspecified own properties to promises. This happens only when debugging. `lockdown()` already prevents this.

## Host objects

The JavaScript spec by definition does not specify the behavior of host objects; that is up to each host platform. However, they are initially accessible only from properties of the realm's `globalThis` object, i.e., from the global object of the implicit start compartment. The JS spec also states that they must obey the same JS object invariants as everything else, with the explicit exception of [`document.all`](https://tc39.es/ecma262/#sec-additional-ecmascript-features-for-web-browsers). For Hardened JS, all I/O or ability to interact with the outside world originates only with such host objects.
...
The JavaScript language can leak the bit encoding of a `NaN` via Data view and shared TypedArray views of a common ArrayBuffer. Although the JavaScript language has only one NaN value, it corresponds to many different bit patterns of the underlying IEEE 754 double-precision floating-point representation. This can be exploited as a side-channel to leak information. This actually happens on some platforms such as V8.

@ChALkeR explains at https://github.com/tc39/ecma262/pull/758#issuecomment-3919093669 the behavior of this side-channel on V8. At https://junk.rray.org/poc/nani.html he demonstrates it, and it is indeed even worse than expected.
...
The JavaScript spec leaves many other observable details not-spec-determined and therefore up to implementation. For example, the prose content of error messages of platform-thrown errors. We agree with this position because we want implementations to be able to improve such prose with minimal friction. The spec does not currently specify that this is only static non-determinism, i.e., that it does not change at runtime within the running of a single program. But de-facto, it is in fact only static, and therefore does not enable the normal kind of non-overt channel. But the prose of error messages are still a non-overt channel that leaks bits that threaten a different security concern: Static non-determinism enables fingerprinting.

Some other de-facto-static potential platform differences
- Locale and time-zone differences can technically vary at runtime. But JS code cannot cause them to vary, and they vary so slowly as to effectively be static channels. In any case, Hardened JS and its ses-shim does replace all the standard `toLocaleString` methods with aliases to the corresponding `toString`. But time-zone info can still leak.
- `for-in` order when the object (and/or its inheritance chain) being enumerated change during enumeration. tc39 has made progress pinning this down, but AFAIK it remains non-deterministic in the spec.
- The default sort algorithm used by `Array.sort`.
